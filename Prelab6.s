...
; Hardware: Led interminente de cada 500ms y un contador decimal con 2 display
;
; Creado: 27 de febrero , 2022
; Última modificación: 28 febrero, 2022
...
INT_TMR0:
    RESET_TMR0  	    ; Reiniciamos TMR0 para 20ms
...
INT_TMR1:
    ...
    BTFSS SEGUNDO,1        ;Saltar a la siguiente liena si es 1
    RETURN
    CLRF SEGUNDO            ;Limpiamos la varible
    INCF SEGUNDO2           ;Se incrementa 
    MOVF SEGUNDO2,W         ;Se mueve a w
    MOVWF CONTA             ;Se mueve a conta
    MOVLW 100               ;Se le agrega 100
    SUBWF CONTA,F           ;Se le resta 100 a CONTA y se guarda en F
    BTFSC ZERO              ;Verificamos la bandera
    CLRF SEGUNDO2           ;Se reseata si esta encendida la bandera
    RETURN
    
INT_TMR2:
    BCF	    TMR2IF	    ; Limpiamos bandera de interrupcion de TMR2
    INCF   VALOR	    ;Se incrementa la variable
    BTFSS VALOR,1           ;Saltar a la siguiente liena si es 1
    RETURN                  ;Volvemos a la interrupcion
    BTFSS VALOR,3           ;Cuando esta a 4 bits
    RETURN
    CLRF VALOR
...
LOOP:
    ...
    CALL    OBTENER_DIVISION	; Obtenemos las decenas y unidades
    GOTO    LOOP	    
...
SET_DISPLAY:
    ...
    MOVWF   display		; Guardamos en display1
    
    MOVF    DECENA, W		; Movemos la variable DECENA a W
    CALL    TABLA_7SEG		; Buscamos valor a cargar en PORTC
    MOVWF   display+1		; Guardamos en display2
...
OBTENER_DIVISION:	        ;    Ejemplo:
    ...
    SUBWF   SEGUNDO1, F		; Se le suma lo de la variable SEGUNDO1 a F
    INCF    DECENA             ; Se incrementa 1 a la variable DECENA
    BTFSC   STATUS,0            ; Se verifica la bandera BOROOW
    
    GOTO    $-4                 ; Si esta encendida la bandera se regresa a 4 instrucciones 
    DECF    DECENA              ; Si no se le resta 1 a DECENA
    
    MOVLW   10	
    ADDWF   SEGUNDO1,F          ; Para hacerlo positivo
...
OBTENER_UNIDADES:	        ;    Ejemplo:
    ...
    SUBWF   SEGUNDO1,F		;
    INCF    UNIDADES             ; Se incrementa 1 a la variable UNIDADES
    BTFSC   STATUS,0            ; Se verifica la bandera BOROOW
